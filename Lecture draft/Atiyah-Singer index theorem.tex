...
% Interior multiplication (formerly supplied by the optional mnsymbol package).
\newcommand{\invneg}{\mathbin{\lrcorner}}
\usepackage{amsmath}
...
\begin{proposition}[Patodi's linear-algebra lemma]
Let $V$ be a finite-dimensional vector space and let
$A_1,\ldots,A_n\in\operatorname{End}(V)$ be pairwise anti-commuting
involutions, where $n=2m$.  If $I=(i_1,\ldots,i_k)$ is an ordered
list of distinct indices and $A_I=A_{i_1}\cdots A_{i_k}$, then
\[
 \Tr(A_1\cdots A_nA_I)=0\qquad (k<n).
\]
If $k=n$, so that $I$ is a permutation of $(1,\ldots,n)$, then
\[
 \Tr(A_1\cdots A_nA_I)=(-1)^m\operatorname{sgn}(I)\dim V.
\]
\begin{proof}
Using $A_j^2=1$ and the anti-commutation relations, the product
$A_1\cdots A_nA_I$ is, up to sign, the Clifford monomial $A_J$ whose
indices form the complement of $I$.  If $J$ is nonempty and proper,
choose $r\in J$ when $|J|$ is even and $r\notin J$ when $|J|$ is odd.
Conjugation by $A_r$ sends $A_J$ to $-A_J$.  Invariance of trace under
conjugation therefore gives $\Tr(A_J)=-\Tr(A_J)=0$.

When $I$ contains all indices, moving its factors into the order
$1,\ldots,n$ contributes $\operatorname{sgn}(I)$, and
\[
 (A_1\cdots A_n)^2=(-1)^{n(n-1)/2}I=(-1)^mI.
\]
Taking traces proves the last assertion.
\end{proof}
\end{proposition}
...
$$
The correct parabolic ansatz is
\[
h(t,x,y)=t^{-n/2}q\left(t^{1/2},x,\frac{x-y}{t^{1/2}}\right).
\]
The first variable $s=t^{1/2}$ records the polyhomogeneous expansion,
while the last variable is the scale-invariant normal variable.
\discussion(continue)
where
$$
...
Here the exponent in the odd case is $-(p+1)/2$: the parity condition
forces one factor of $t^{1/2}$ after integration. Thus $p\leq -3$ is
indeed the range in which the result vanishes at $t=0$.
...
\begin{proposition}[Taylor expansion with parity]
For every $N\in\N$,
\[
\tilde q(x,s,\omega)=
 \sum_{k=0}^{2N-1}\frac{s^k}{k!}
 (\partial_s^k\tilde q)(x,0,\omega)
 {}+s^{2N}r_{2N}(x,s,\omega),
\]
where $r_{2N}$ is smooth and remains Schwartz in $\omega$, locally
uniformly in $x$. Moreover,
\[
(\partial_s^k\tilde q)(x,0,-\omega)
 =(-1)^{p+k}(\partial_s^k\tilde q)(x,0,\omega).
\]
\begin{proof}
Taylor's formula with integral remainder gives the expansion and
preserves the Schwartz estimates. Differentiate
$\tilde q(x,-s,-\omega)=(-1)^p\tilde q(x,s,\omega)$ $k$ times and set
$s=0$ to obtain the parity assertion.
\end{proof}
\end{proposition}

\discussion
Suppose $p$ is even. The integrals of the odd Taylor coefficients
vanish, so the preceding proposition gives
\[
Qu(x,t)=t^{-\frac p2-1}
 \sum_{k=0}^{N-1}t^k f_{2k}(x)
 {}+t^{-\frac p2-1+N}F_{2N}(x,t),
\]
where
\[
f_{2k}(x)=\frac1{(2k)!}\int_{\R^n}
 (\partial_s^{2k}\tilde q)(x,0,\omega)\,d\omega,
\]
and $F_{2N}$ is smooth down to $t=0$. In conclusion,
\[
Qu(t,x)\in t^{-\frac{p}{2}-1}C^{\infty}([0,\infty)_{t}\times \R^{n})
\]
\execrise
...
+
$$
\partial_t:\Psi_h^{p}\rightarrow \Psi^{p+2}_{h}
$$
+
$$
...
\begin{proof}
For the inclusion, write the same kernel with amplitude
$\widetilde q(s,x,\omega)=s^kq(s,x,\omega)$. It is still Schwartz and
\[
 \widetilde q(-s,x,-\omega)=(-1)^{p+k}\widetilde q(s,x,\omega),
\]
so $Q\in\Psi_n^{p+k}$.

Put $a=(n+p+2)/2$, $s=t^{1/2}$ and
$\omega=(x-y)/s$. Differentiation under the integral gives
\[
 \partial_t Q=t^{-a-1}\int
 \left(-a q+\frac{s}{2}\partial_s q
             -\frac12\omega\cdot\partial_\omega q\right)u(y)\,dy.
\]
The amplitude in parentheses has the required parity for
$\Psi_n^{p+2}$. Similarly,
\[
 \partial_{x_j}Q=t^{-a-1/2}\int
 \left(s\partial_{x_j}q+\partial_{\omega_j}q\right)u(y)\,dy
 \in\Psi_n^{p+1}.
\]
Iterating the last identity and applying the Leibniz rule to smooth
coefficients proves $P\Psi_n^p\subseteq\Psi_n^{p+m}$ for every
differential operator $P$ of order at most $m$.
\end{proof}

\definition
Let
\[
 \mathcal S_p=\{a(x,\omega)\in\mathbb S^{-\infty}(\R^n_x;\R^n_\omega):
 a(x,-\omega)=(-1)^pa(x,\omega)\}.
\]
For $Q\in\Psi_n^p$ represented by the amplitude $q$, define its leading
heat symbol by
\[
 \sigma_p(Q)(x,\omega)=q(0,x,\omega)\in\mathcal S_p.
\]

\begin{theorem}[Exact heat-symbol sequence]
The sequence
\[
0\longrightarrow\Psi_n^{p-1}\longrightarrow\Psi_n^p
 \xrightarrow{\ \sigma_p\ }\mathcal S_p\longrightarrow0
\]
is exact.
\begin{proof}
The parity condition puts $q(0,x,\cdot)$ in $\mathcal S_p$.
Surjectivity follows by extending any $a\in\mathcal S_p$ constantly in
$s$ near zero and multiplying by an even cutoff in $s$. If
$\sigma_p(Q)=0$, Hadamard's lemma gives $q=sq_1$ with $q_1$ smooth,
Schwartz in $\omega$, and of parity $p-1$. Rewriting the prefactor
shows $Q\in\Psi_n^{p-1}$. The converse is the same calculation in
reverse.
\end{proof}
\end{theorem}
\discussion
...
\begin{proof}
If $H$ exists, define the Volterra operator
\[
 (Gg)(t)=\int_0^t H(t-s)g(s)\,ds.
\]
Leibniz' rule, $H(0)=I$, and $(\partial_t+L)H=0$ give
\[
 (\partial_t+L)Gg
 =g(t)+\int_0^t(\partial_t+L)H(t-s)g(s)\,ds=g(t),
\]
and the integral is zero at $t=0$.

Conversely, suppose $G$ has the stated properties. For
$u\in C^\infty(M)$ let $\iota u$ denote the time-independent function
$(\iota u)(t,x)=u(x)$ and set
\[
 Hu=\partial_tG(\iota u).
\]
Because $L$ is independent of $t$, differentiating
$(\partial_t+L)G(\iota u)=\iota u$ gives
$(\partial_t+L)Hu=0$. At $t=0$,
\[
 u=((\partial_t+L)G(\iota u))(0)
   =Hu(0)+L(G(\iota u)(0))=Hu(0),
\]
since $G(\iota u)(0)=0$. Thus $H(0)=I$.
\end{proof}
...
\corollary
The heat kernel exists.
\proof
Set $G=\tilde Q\circ(I+\tilde{\RR})^{-1}$. The preceding identities
give $(\partial_t+L)G=I$. Both factors are Volterra operators, hence
$Gg|_{t=0}=0$ for every $g$. Duhamel's principle therefore yields
\[
 H=\partial_tG,\qquad (\partial_t+L)H=0,\qquad H|_{t=0}=I.
\]
Since the inverse differs from the identity by a smooth Volterra
operator flat at $t=0$, the constructed $H$ differs from the
parametrix $Q$ by an element of $\Psi_{\HH}^{-\infty}(M)$.
...
&=Id+\sum (-C_{f})^{k}\\
&=Id+\sum^{\infty}_{k=1}(-1)^k C_{f^{*k}}
\end{align}

We still need to address the issue of convergence. For this we have the following theorem:

\begin{theorem}[Volterra-series convergence]
Let $h\in C^{0}([0,\infty)\times M\times M)$ and write $h^{*k}$ for
the $k$-fold Volterra convolution. Then
\[
j(t,x,y)=\sum_{k=1}^{\infty}(-1)^k h^{*k}(t,x,y)
\]
converges uniformly on $[0,t_0]\times M\times M$ for every $t_0>0$.
The limit is continuous. If $h$ is smooth, then $j$ is smooth; if $h$
is flat at $t=0$, then so is $j$.
\begin{proof}
...
$$
|h^{*k}(t,x,y)|\le
\frac{t^{k-1}\operatorname{vol}(M)^{k-1}\|h\|_{\infty}^{k}}{(k-1)!}
$$
Indeed, $h^{*k}$ is an integral over $M^{k-1}$ and the time simplex
\[
 \{(s_1,\ldots,s_{k-1}):s_i\geq0,\ s_1+\cdots+s_{k-1}\leq t\}.
\]
The spatial integral contributes at most
$\operatorname{vol}(M)^{k-1}$ and the simplex has volume
$t^{k-1}/(k-1)!$, which proves the estimate.
\discussion
Observe that the right hand side equal
$$
\|h\|_{\infty}
 \exp\!\bigl(t_0\operatorname{vol}(M)\|h\|_{\infty}\bigr)<\infty
$$
Therefore
$$
\sum_{k\geq1}(-1)^k h^{*k}
...
j=-h-h*j=-h-j*h
$$
Thus $(Id+C_f)^{-1}=Id+C_j$ exists. Taking $h=f$, write $g=j$.
$$
g(t,x,y)=\sum^{\infty}_{k=1}(-1)^k f^{*k}
 \in C^{\infty}([0,\infty)\times M\times M)
$$
and converges uniformly on $[0,t_0]\times M\times M$, for all $t_0>0$.
The same estimates after differentiation prove smooth convergence when
$f$ is smooth. If $f$ is flat at $t=0$, the convolution estimates
applied to every derivative show that $g$ is flat as well.
\end{proof}
\end{theorem}
...
where
$$
\tilde{a}_{k}(x)=\frac{\partial_{s}^{2k}q(x,0,0)}{(2k)!}
$$
So we have
\begin{align}
...
Therefore
$$
t^{n/2}\Tr(Q)\sim\sum^{\infty}_{k=0}t^{k}a_{k},
\qquad a_k=\int_M\tilde a_k(x)\,dg(x)
$$
\proof(continue)
Thus we have
$$
...
We have
$$
\Tr(e^{-t\D^{-}\D^{+}})\sim\sum^{\infty}_{k=0}t^{-\frac{n}{2}+k}a_{k}^{+}
$$
as well as
$$
\Tr(e^{-t\D^{+}\D^{-}})\sim\sum^{\infty}_{k=0}t^{-\frac{n}{2}+k}a_{k}^{-}
...
$$
a_{k}^{\pm}=\frac1{k!}
 \left.\left(\frac{d}{dt}\right)^{k}
 \left[t^{\frac n2}\Tr(e^{-t\D^{\mp}\D^{\pm}})\right]\right|_{t=0}
$$
...
\IND=a_{m}^{+}-a_{m}^{-}
$$
where the difference is the coefficient of $t^m$ in the Taylor
expansion of
\[
t^{m}\left(\Tr(e^{-t\D^{-}\D^{+}})
          -\Tr(e^{-t\D^{+}\D^{-}})\right).
...
\begin{lemma}[Clifford relation]
For all $\xi,\eta\in V$,
\[
 \sigma(\xi)\sigma(\eta)+\sigma(\eta)\sigma(\xi)
 =2\langle\xi,\eta\rangle I.
\]
In particular, the two endomorphisms anti-commute when
$\xi\perp\eta$.
\begin{proof}
By linearity and the identity $\sigma(\zeta)^2=|\zeta|^2I$,
\[
\begin{aligned}
|\xi+\eta|^2I
 &=\sigma(\xi+\eta)^2\\
...
\end{aligned}
\]
Subtracting $(|\xi|^2+|\eta|^2)I$ from both sides proves the formula.
\end{proof}
\end{lemma}
\discussion
...
$$
\begin{theorem}[Vector-space structure of the Clifford algebra]
Let $\operatorname{Cl}_{\C}(V)$ be the universal complex algebra
generated by $V$ with relations
\[
 vw+wv=2\langle v,w\rangle.
\]
If $e_1,\ldots,e_n$ is an orthonormal basis, then the monomials
$e_{i_1}\cdots e_{i_k}$ with $i_1<\cdots<i_k$ form a basis. Hence the
symbol map gives a vector-space isomorphism
\[
 \operatorname{Cl}_{\C}(V)\cong\C\otimes\bigwedge\nolimits^*V.
\]
This is not an isomorphism of algebras: the Clifford product corresponds
to exterior multiplication plus contraction.
\begin{proof}
The Clifford relations reorder every word into a linear combination of
the displayed monomials, so they span. Filter the Clifford algebra by
word length. Its associated graded algebra is a quotient of
$\bigwedge^*V$; conversely the standard exterior-algebra representation
$c(v)=\varepsilon(v)+\iota(v)$ has these monomials as independent
principal symbols. Thus the quotient map on associated graded spaces is
an isomorphism, proving linear independence.
\end{proof}
\end{theorem}
...
\begin{theorem}[Joint eigenspace decomposition]
For $\lambda=(\lambda_1,\ldots,\lambda_m)\in\{\pm1\}^m$, set
\[
 W_\lambda=\{w\in W:c_kw=\lambda_kw\text{ for every }k\}.
\]
Then
\[
 W=\bigoplus_{\lambda\in\{\pm1\}^m}W_\lambda,
 \qquad \dim W_\lambda=\frac{\dim W}{2^m}.
\]
\begin{proof}
The operators $c_k$ are commuting self-adjoint involutions, hence the
spectral theorem gives their simultaneous orthogonal eigenspace
decomposition, with eigenvalues in $\{\pm1\}$. Let
$\lambda^{(k)}$ be obtained from $\lambda$ by changing the sign of its
$k$th coordinate. For $w\in W_\lambda$,
\[
 c_j(A_kw)=
 \begin{cases}
  -A_kc_kw=-\lambda_kA_kw,&j=k,\\
  A_kc_jw=\lambda_jA_kw,&j\ne k.
 \end{cases}
\]
Thus $A_k:W_\lambda\to W_{\lambda^{(k)}}$. Since $A_k^2=I$, this map is
an isomorphism. Any two sign vectors are joined by a sequence of such
sign changes, so all $2^m$ joint eigenspaces have the same dimension.
Their direct sum is $W$, which gives the stated value.
\end{proof}
\end{theorem}
...
A holomorphic $p$-form means a holomorphic section of
$\Omega_M^p=\wedge^{p,0}T^*M$. Thus it is a form of type $(p,0)$
which, in every holomorphic chart, has the form
\[
 \alpha=\sum_{|I|=p}a_I(z)\,dz_I
\]
with holomorphic coefficients $a_I$; equivalently,
$\overline\partial\alpha=0$. For $q>0$, a
$\overline\partial$-closed $(p,q)$-form is a Dolbeault cocycle, not a
``holomorphic $(p,q)$-form''.

\section{Dolbeault complex}
For each $p$, the Dolbeault complex is
\[
0\longrightarrow A^{p,0}(M)
 \xrightarrow{\overline\partial}A^{p,1}(M)
 \xrightarrow{\overline\partial}\cdots
 \xrightarrow{\overline\partial}A^{p,m}(M)\longrightarrow0,
\]
where $A^{p,q}(M)=C^\infty(M,\wedge^{p,q}T^*M)$. Its cohomology is
\[
 H_{\overline\partial}^{p,q}(M)
 =\frac{\ker(\overline\partial:A^{p,q}\to A^{p,q+1})}
        {\operatorname{im}(\overline\partial:A^{p,q-1}\to A^{p,q})}.
\]
The holomorphic Euler characteristic of the structure sheaf is
\[
 \chi(M,\mathcal O_M)
 =\sum_{q=0}^m(-1)^q\dim H_{\overline\partial}^{0,q}(M).
\]

From our work of almost complex manifolds we know that

\theorem If $M$ is a compact complex manifold, then
\[
 \chi(M,\mathcal O_M)=\int_M\operatorname{Td}(T^{1,0}M).
\]
For a Hermitian connection with curvature matrix $K$, our convention is
\[
 \operatorname{Td}(T^{1,0}M)
 =\det\left(\frac{\frac{i}{2\pi}K}
 {I-\exp(-\frac{i}{2\pi}K)}\right),
\]
and the integral means the top-degree component.

\proof By our theorem on Dirac complexes, the Dolbeault cohomology 
\[
H_{\overline\partial}^{0,k}(M)\cong
 \ker\!\left(\overline\partial+\overline\partial^{\,*}\right)
 \big|_{A^{0,k}(M)}
\]
Here $\overline\partial^{\,*}$ is the formal adjoint of
$\overline\partial$ with respect to the chosen Hermitian metric. We know that
...
We thus have
\[
\chi(M,\mathcal O_M)=\operatorname{Ind}D_d^+
 =\int_M\operatorname{Td}(T^{1,0}M)
\]

This is the Hirzebruch--Riemann--Roch theorem for the structure
sheaf. Hodge theory identifies Dolbeault cohomology with harmonic forms,
and the index theorem evaluates the resulting Dolbeault--Dirac index by
the Todd class.
